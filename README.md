...
Changelog
---------
- version 0.74, 12. of January 2025: make compatible to current modbus tcp python library
...
services:
  modbus2mqtt:
    volumes:
      - ./eastron_sdm72.csv:/app/conf/modbus2mqtt.csv
    image: modbus2mqtt
    container_name: Modbus2MQTT
    command: --tcp <ip_address> --tcp-port <port> --mqtt-host <ip_address> --mqtt-port <port> --mqtt-user <username> --mqtt-pass <password>
```
